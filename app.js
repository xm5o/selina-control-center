(() => {
  ...
  async function refreshAll(){try{const days=state.analyticsDays||30;const [overview,music,guilds,system,logs,commands,diagnostics,summary,daily,health]=await Promise.all([api('/api/overview'),api('/api/music'),api('/api/guilds'),api('/api/system'),api('/api/logs?limit=160'),api(`/api/analytics/commands?days=${days}`),api(`/api/diagnostics?days=${days}`),api(`/api/analytics/summary?days=${days}`),api(`/api/analytics/daily?days=${days}`),api(`/api/analytics/health?days=${Math.min(days,30)}`)]);Object.assign(state,{overview,music,guilds:guilds.guilds||[],system,events:logs.events||[],commands,diagnostics,summary,daily,health});render();setLive(true)}catch(e){console.warn('[Dashboard]',e);setLive(false)}}
  ...
  function switchView(v){document.querySelectorAll('.nav').forEach(b=>b.classList.toggle('active',b.dataset.view===v));document.querySelectorAll('.view').forEach(x=>x.classList.toggle('active',x.id===`view-${v}`));$('pageTitle').textContent=({overview:'Overview',servers:'Servers',users:'Users',commands:'Commands',activity:'Activity',music:'Music',trends:'Trends',diagnostics:'Diagnostics',system:'Runtime'})[v]||'Selina'}
