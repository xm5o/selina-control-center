(() => {
  ...
  async function refreshAll(){try{const days=state.analyticsDays||30;const [overview,music,guilds,system,logs,commands,diagnostics,summary,daily,health,capabilities,audit]=await Promise.all([api('/api/overview'),api('/api/music'),api('/api/guilds'),api('/api/system'),api('/api/logs?limit=160'),api(`/api/analytics/commands?days=${days}`),api(`/api/diagnostics?days=${days}`),api(`/api/analytics/summary?days=${days}`),api(`/api/analytics/daily?days=${days}`),api(`/api/analytics/health?days=${Math.min(days,30)}`),api('/api/controls/capabilities'),api('/api/audit?limit=60')]);Object.assign(state,{overview,music,guilds:guilds.guilds||[],system,events:logs.events||[],commands,diagnostics,summary,daily,health,capabilities,audit});render();setLive(true)}catch(e){console.warn('[Dashboard]',e);setLive(false)}}
  ...
  async function openGuild(id){const b=$('guildDetail');b.innerHTML='<article class="panel server-detail">Loading…</article>';try{const [g,a]=await Promise.all([api(`/api/guilds/${id}`),api(`/api/guilds/${id}/analytics?days=${state.analyticsDays||30}`)]);const top=(a.commands||[]).map(x=>`<span class="chip">/${escapeHtml(x.command)} · ${x.uses}</span>`).join('')||'<span class="chip">No command history</span>';b.innerHTML=`<article class="panel server-detail"><div class="head"><div><p class="eyebrow">SERVER DETAIL</p><h3>${escapeHtml(g.name)}</h3></div><span class="pill">${g.members} members</span></div><div class="history-grid"><div><span>Events</span><b>${a.totals?.events||0}</b></div><div><span>Commands</span><b>${a.totals?.commands||0}</b></div><div><span>Music</span><b>${a.totals?.music||0}</b></div><div><span>Active users</span><b>${a.totals?.activeUsers||0}</b></div><div><span>Last active</span><b>${a.totals?.lastActivityAt?ago(a.totals.lastActivityAt):'—'}</b></div></div><p class="eyebrow" style="margin-top:18px">TOP COMMANDS</p><div class="chips">${top}</div><div class="columns" style="margin-top:18px"><div><p class="eyebrow">CHANNELS · ${g.channels.length}</p><div class="list">${g.channels.map(c=>`<div># ${escapeHtml(c.name)}</div>`).join('')}</div></div><div><p class="eyebrow">ROLES · ${g.roles.length}</p><div class="list">${g.roles.map(r=>`<div>${escapeHtml(r.name)}${r.managed?' · managed':''}</div>`).join('')}</div></div></div></article>`}catch(e){b.innerHTML=`<article class="panel">${escapeHtml(e.message)}</article>`}}
  ...
  function switchView(v){document.querySelectorAll('.nav').forEach(b=>b.classList.toggle('active',b.dataset.view===v));document.querySelectorAll('.view').forEach(x=>x.classList.toggle('active',x.id===`view-${v}`));$('pageTitle').textContent=({overview:'Overview',servers:'Servers',users:'Users',commands:'Commands',activity:'Activity',music:'Music',controls:'Controls',trends:'Trends',diagnostics:'Diagnostics',system:'Runtime'})[v]||'Selina'}
